 ...
 *
 * Stats API Worker - Handles /api/stats endpoint for download statistics.
 *
 * This module provides the fetch handler for the stats API. It reads from
 * the D1 database and returns aggregated download statistics with caching.
 *
 * Endpoints:
...
/**
 * Worker environment bindings for the stats API.
 */
export interface StatsApiEnv {
 ...
 */
async function handleStats(request: Request, env: StatsApiEnv): Promise<Response> {
    if (!env.STATS_DB) {
...
export default {
    /**
     * Fetch handler for the stats API.
     *
     * Routes:
     *   OPTIONS /api/*  - CORS preflight
     *   GET /api/stats  - Download statistics
     *   *               - 404 Not Found or fallback to assets
     */
    async fetch(request: Request, env: StatsApiEnv): Promise<Response> {
        const url = new URL(request.url);
