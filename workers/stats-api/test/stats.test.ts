...
async function clearDatabase(): Promise<void> {
  await env.STATS_DB.prepare("DELETE FROM release_assets").run();
...
describe("Stats API Worker", () => {
  ...
  describe("OPTIONS /api/stats", () => {
    it("should return CORS headers", async () => {
      // Arrange & Act
      const response = await worker.fetch(new Request("http://localhost/api/stats", { method: "OPTIONS" }), env);
  ...
  describe("Error Handling", () => {
    ...
    it("should return 404 for unknown API routes", async () => {
      ...
      const response = await worker.fetch(new Request("http://localhost/api/unknown"), env);
