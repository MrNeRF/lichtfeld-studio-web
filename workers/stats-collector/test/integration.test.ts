...
describe("Stats Collector Integration (Real GitHub API)", () => {
  ...
  describe("MrNeRF/LichtFeld-Studio", () => {
    ...
    it("should store fetched releases in database", async () => {
      ...
      const dbReleases = await env.STATS_DB.prepare("SELECT COUNT(*) as count FROM releases").first<{
        count: number;
      }>();
      ...
      if (body.releasesFound > 0) {
        const releases = await env.STATS_DB.prepare("SELECT tag, name, total_downloads FROM releases").all<{
          tag: string;
          name: string;
